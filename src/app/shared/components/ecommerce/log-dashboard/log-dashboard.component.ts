...
export class LogDashboardComponent  implements OnInit, OnDestroy {
  ...
  readonly datePresets: DatePreset[] = [
    { label: 'Today',        value: 'today', ...this.buildPreset(0, 0)  },
    { label: 'Last 7 days',  value: '7d',    ...this.buildPreset(6, 0)  },
    { label: 'Last 30 days', value: '30d',   ...this.buildPreset(29, 0) },
    { label: 'Last 90 days', value: '90d',   ...this.buildPreset(89, 0) },
  ];

  // ─── Computed ─────────────────────────────────────────────
  filteredInstances = computed(() =>
    this.instances().filter(i =>
      i.host.toLowerCase().includes(this.instanceSearch().toLowerCase())
    )
  );
  ...
  kpiCards = computed(() => {
    ...
    return [
      ...
      {
        id: 'exception', label: 'Top Exception',
        value: s.topExceptionType ? (s.topExceptionType.split('.').pop() ?? 'None') : 'None',
        rawValue: 0, icon: 'bug', color: 'red', delta: null,
        sub: s.topExceptionType ?? 'No exceptions'
      }
    ];
  });
...
private getEnvironmentLabel(env: number): string {
  ...
  private buildTimelineChart(logs: LogTimeSeries[], errors: LogTimeSeries[], prevLogs?: LogTimeSeries[]): void {
    ...
    this.charts['timeline'] = new Chart(canvas, {
      ...
      options: {
        ...
        scales: {
          x: { ticks: { color: '#64748b', maxTicksLimit: 12, font: { size: 11 } }, grid: { color: 'rgba(100,116,139,0.1)' } },
  ...
  private buildLevelPieChart(levels: any[]): void {
    ...
    const colorMap: Record<string, string> = {
      Error: '#ef4444', Fatal: '#7c3aed', Warning: '#f59e0b',
      Information: '#3b82f6', Debug: '#6b7280', Trace: '#0ea5e9', Unknown: '#374151'
    };
    ...
    const chartColors = hasData ? levels.map(l => colorMap[l.level] ?? '#6b7280') : ['#1e293b'];
  ...
  private buildServicesChart(services: ServiceStats[]): void {
    ...
    this.charts['services'] = new Chart(canvas, {
      ...
      data: {
        labels,
        datasets: [
          { label: 'Total Logs', data: logData, backgroundColor: 'rgba(59,130,246,0.7)', borderRadius: 4, borderSkipped: false },
          { label: 'Errors',     data: errData, backgroundColor: 'rgba(239,68,68,0.7)',  borderRadius: 4, borderSkipped: false }
        ]
      },
      options: {
        ...
        scales: {
          x: { ticks: { color: '#64748b' }, grid: { color: 'rgba(100,116,139,0.1)' }, beginAtZero: true },
  ...
  private buildExceptionsChart(exceptions: ExceptionStats[]): void {
    ...
    const colors = top8.length ? top8.map((_, i) => `hsl(${360 - i * 30}, 70%, 55%)`) : ['#1e293b'];
    ...
    this.charts['exceptions'] = new Chart(canvas, {
      ...
      options: {
        ...
        scales: {
          x: { ticks: { color: '#94a3b8', font: { size: 10 }, maxRotation: 35 }, grid: { display: false } },
          y: { ticks: { color: '#64748b' }, grid: { color: 'rgba(100,116,139,0.1)' }, beginAtZero: true }
        }
      }
    });
  ...
  private buildDurationChart(buckets: DurationBucket[]): void {
    ...
    this.charts['duration-buckets'] = new Chart(canvas, {
      ...
      data: {
        labels,
        datasets: [{ label: '% of requests', data, backgroundColor: ['#22c55e','#84cc16','#f59e0b','#f97316','#ef4444'], borderRadius: 6, borderSkipped: false }]
      },
      options: {
        responsive: true, maintainAspectRatio: false,
        plugins: {
        ...
        scales: {
          ...
          y: { ticks: { color: '#64748b', callback: v => `${v}%` }, grid: { color: 'rgba(100,116,139,0.1)' }, beginAtZero: true }
        }
      }
    });
  }
    ...
    this.charts['duration-time'] = new Chart(canvas, {
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: { backgroundColor: '#1e293b', callbacks: { label: item => ` ${item.raw}ms` } }
        },
        scales: {
          x: { ticks: { color: '#64748b', maxTicksLimit: 10 }, grid: { color: 'rgba(100,116,139,0.1)' } },
          y: { ticks: { color: '#64748b', callback: v => `${v}ms` }, grid: { color: 'rgba(100,116,139,0.1)' }, beginAtZero: true }
        }
      }
    });
  }
