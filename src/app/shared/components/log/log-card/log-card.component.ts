...
@Component({
  ...
  styles: [`
    .ai-btn {
      animation: ai-idle-pulse 3s ease-in-out infinite;
    }
    .ai-btn:hover, .ai-btn:focus-visible {
      animation: none;
      box-shadow: 0 6px 24px rgba(135, 136, 255, 0.28);
      border-color: rgba(135, 136, 255, 0.5) !important;
    }
    .ai-btn:disabled {
      animation: none;
    }
    @keyframes ai-idle-pulse {
      0%, 100% { box-shadow: 0 0 0 0 rgba(135, 136, 255, 0); }
      50%       { box-shadow: 0 0 0 5px rgba(135, 136, 255, 0.10); }
    }
    .ai-arrow {
      transition: transform 0.2s ease;
    }
    .ai-btn:hover .ai-arrow {
      transform: translateX(3px);
    }
    @media (prefers-reduced-motion: reduce) {
      .ai-btn { animation: none; }
      .ai-arrow { transition: none; }
    }
  `]
})
